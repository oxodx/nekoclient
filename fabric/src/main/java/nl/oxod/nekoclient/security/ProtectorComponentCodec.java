...
public record ProtectorComponentCodec(Codec<Component> wrapped) implements Codec<Component> {
  @Override
  public <T> DataResult<Pair<Component, T>> decode(DynamicOps<T> ops, T input) {
    DataResult<Pair<Component, T>> result = wrapped.decode(ops, input);
    if (!Protector.shouldTagPacketComponents()) return result;
    if (!ProtectorPacketContext.isProcessingPacket()) return result;
    Minecraft mc;
    try {
  ...
  private static void markTree(Component component) {
    ...
    if (contents instanceof ProtectorFromPacketAccess access) access.protector$setFromPacket();
